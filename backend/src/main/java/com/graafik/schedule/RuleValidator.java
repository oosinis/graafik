...
public class RuleValidator {
     ...
     * @param currentSchedule            the schedule that has already been checked
     *                                   and saved
     * @param currentDayShiftAssignments new assignments to be checked now
     * @return the score of the new assignments
     */
    public static int dayAssignmentsValidator(ScheduleRequest scheduleRequest, ScheduleAlg currentSchedule,
        ...
        for (ShiftAssignment shiftAssignment : currentDayShiftAssignments.getAssignments()) {
            ...
            int newDayScheduleDate = (currentSchedule.getDaySchedules() == null) ? 0
                    : currentSchedule.getDaySchedules().size();
            ...
            int countCont = checkContinuousNewAssignment(shiftAssignment, currentSchedule, currentDayShiftAssignments,
                    newDayScheduleDate - 1);
     ...
     * @param currentSchedule            the schedule tht' already been checked and
     *                                   saved
     * @param currentDayShiftAssignments the new assignments we re currently
     *                                   checking, here just to chnge the score if
     *                                   the continuous shifts are the exact
     *                                   required amount
     * @param date
     * @return the nr of continuous shifts, if -1 then there are too many
     */
    private static int checkContinuousNewAssignment(ShiftAssignment shiftAssignment, ScheduleAlg currentSchedule,
            DaySchedule currentDayShiftAssignments, int date) {
        int countCont = 0;
    ...
    /**
     * check how many of the previously assigned hift in a row to know the required
     * rest time
     * 
    ...
    public static void checkDesiredVacationDays(Employee employee, int date, ScheduleAlg currentSchedule,
        ...
        if (HelperMethods.isDateInList(employee.getVacationDays(), year, month, date)) {
            currentDayShiftAssignments.addToScore(-10);
        }
        if (HelperMethods.isDateInList(employee.getSickDays(), year, month, date)) {
            currentDayShiftAssignments.addToScore(-10);
        }
    }

    public static void checkEmployeeMinutes(ShiftAssignment shiftAssignment, ScheduleAlg currentSchedule,
            DaySchedule currentDayShiftAssignments) {
        Employee employee = shiftAssignment.getEmployee();
        ...
        // rn in inutes, 2h mööda on fine, peale seda võtab maha skoorist 2x nii plju
        // kui palju tunde mööda vajalikes
    ...
    public static boolean initialValidator(Map<UUID, List<Employee>> currentRequestedWorkDays, Shift shift,
            Employee employee) {
        return employee.getAssignedShifts().contains(shift) || currentRequestedWorkDays.containsKey(shift.getId())
                && !currentRequestedWorkDays.get(shift.getId()).contains(employee);
    }
     ...
     */
    public static int singleAssignmentValidator(ScheduleRequest scheduleRequest, ScheduleAlg currentSchedule,
            ShiftAssignment shiftAssignment, int date) {
        ...
        long newHours = currentSchedule.getEmployeeHoursInMinutes().get(employee.getId())
     ...
     * @return
     */
    private static int checkContinuousNewAssignmentSingleShift(ShiftAssignment shiftAssignment,
            ScheduleAlg currentSchedule, int date) {
        int countCont = 0;
        List<Rule> rules = shiftAssignment.getShift().getRules();
        Employee employee = shiftAssignment.getEmployee();
