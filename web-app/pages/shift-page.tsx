...
async function getSchedule(year: number, month: number): Promise<ScheduleResponse | null> {
    ...
    return {
      ...
      daySchedules: Array.from({ length: daysInMonth }, (_, dayIndex) => {
        const assignments = shiftTypes.map((type) => {
          const assignment = {
            shift: { type, length: type === "Öö" ? 10 : 8, duration: (type === "Öö" ? 600 : 480), rules: [] },
  ...
  return {
    ...
    daySchedules: Array.from({ length: daysInMonth }, (_, i) => ({
      ...
      score: Math.floor(Math.random() * 100),
      assignments: [
        { shift: { type: "Hommik", length: 8, duration: 480, rules: [] }, worker: { name: "Jaan Tamm" } },
        { shift: { type: "Päev", length: 8, duration: 480, rules: [] }, worker: { name: "Mari Mets" } },
        { shift: { type: "Õhtu", length: 8, duration: 480, rules: [] }, worker: { name: "Peeter Kask" } },
        { shift: { type: "Öö", length: 8, duration: 480, rules: [] }, worker: { name: "Liis Lepp" } },
      ],
    })),
  }
}

async function getScheduleData(year: number, month: number) {
...
export default async function ScheduleDetailPage({ params }: { params: { year: string; month: string } }) {
  ...
  const schedule = await getScheduleData(year, month)
