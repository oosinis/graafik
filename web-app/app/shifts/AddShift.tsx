...
export function AddShift({ onSave, onDiscard, editingShift }: AddShiftProps) {
  ...
  const [startTime, setStartTime] = React.useState(editingShift?.startTime ?? "09:00");
  const [endTime, setEndTime] = React.useState(editingShift?.endTime ?? "17:00");
  const [shiftType, setShiftType] = React.useState<Shift["type"]>(editingShift?.type ?? "Day");
  ...
  const handleAddRule = () => {
    ...
    setRuleForm({ name: "", daysApplied: [], priority: "medium", perDay: "1", continuousDays: "5", restDays: "2" });
  ...
  const inputClass = "bg-[#f7f6fb] h-[40px] px-[14px] rounded-[8px] text-[15px] w-full outline-none";

  return (
    <div className="py-8 w-full">
      {/* Header */}
      <div className="mb-8 flex justify-between items-center pr-6">
        <h1 className="text-[24px] font-medium">{editingShift ? "Edit Shift" : "Create New Shift"}</h1>
        <div className="flex gap-2">
          <button onClick={onDiscard} className="text-[#7636ff] px-4 py-2 rounded hover:bg-[#f7f6fb]">
            Cancel
          </button>

          <button
            onClick={handleSave}
            className="bg-[#7636ff] text-white px-6 py-2 rounded flex gap-2 items-center hover:bg-[#6428e0]"
          >
            <Check size={18} />
      ...
      <div className="bg-white p-6 rounded-lg max-w-4xl shadow-sm">
        <div className="flex justify-between items-center">
          ...
          {!isAddingRule && (
            <button
              ...
              className="bg-[#7636ff] text-white px-4 py-2 rounded flex items-center gap-2 hover:bg-[#6428e0]"
            >
        ...
        {rules.length > 0 && (
          <div className="mt-4 grid gap-3">
            {rules.map((r) => (
              <div key={r.id} className="bg-[#f7f6fb] p-3 rounded-lg flex justify-between">
                <div>
                  <p className="font-medium">{r.name}</p>
                  <p className="text-xs text-[#888796] mt-1">
                    {r.daysApplied.join(", ")} • {r.perDay} per day • {r.continuousDays} days • {r.restDays} off
                  </p>
                </div>
        ...
        {isAddingRule && (
          <div className="bg-[#f7f6fb] p-5 mt-4 rounded-lg grid gap-4">
                ...
                {[1, 2, 3, 4, 5, 6, 7].map((d) => (
                  <button
                    ...
                    className={`px-3 py-1 rounded ${
                      ruleForm.daysApplied.includes(d) ? "bg-[#7636ff] text-white" : "bg-white text-[#888796]"
                    }`}
                  >
                    {d}
                  </button>
            ...
            <div className="flex justify-end gap-2">
              <button onClick={() => setIsAddingRule(false)} className="text-[#888796] px-3 py-1">
                Cancel
              </button>
              <button onClick={handleAddRule} className="bg-[#7636ff] text-white px-3 py-1 rounded">
                Add Rule
              </button>
            </div>
          </div>
