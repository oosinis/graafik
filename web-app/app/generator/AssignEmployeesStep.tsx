...
export function AssignEmployeesStep({
  ...
  const saveNewWorker = () => {
    ...
    const worker: Worker = {
      ...
      email: undefined,
      phone: undefined,
      assignedShifts: draftAssignedShiftIds, //muutsin selle koha ära ja modelis ka, aga palju erroreid niiet pmst kogu kood vaja sellest perspektiivist üle vaadata
      ...
      <div className="rounded-lg bg-purple-50 p-4 space-y-6 animate-in fade-in-50">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {shifts.map(s => {
              const on = draftAssignedShiftIds.includes(s.id) //ei ole kindel kas includes on siin õige
              return (
                <Button
                  key={s.id}
        ...
        <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {Array.from({ length: daysInMonth }, (_, i) => i + 1).map(day => {
              ...
              const assignedForDraft = shifts.filter(s => draftAssignedShiftIds.includes(s.id))//idk kas includes on õige siin
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {shifts.map(s => {
                const on = (activeWorker.assignedShifts ?? []).some(as => as === s.id)
                return (
                  <Button
                    key={s.id}
                    variant={on ? "default" : "outline"}
