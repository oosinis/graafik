    ...
    async function load() {
      try {
        ...
        if (scheduleId) {
          ...
          res = await fetch(`${apiUrl}/schedules/latest?month=${month}&year=${year}`);
        }
        ...
        if (data.daySchedules && data.daySchedules.length > 0) {
          ...
          console.log("First day assignments:", data.daySchedules[0]?.assignments);
        }
        if (!cancelled) {
          setSchedule(data);
    ...
    <div className="w-full space-y-4">
      <div className="flex justify-between items-center">
        <MonthNavigation
          ...
          onPrevious={() => setCurrentDate(d => new Date(d.getFullYear(), d.getMonth() - 1, 1))}
      ...
      {!schedule && noScheduleFound && (
        <div className="text-center py-8">
          <p className="text-gray-500 text-lg">No Schedule for this month yet</p>
        </div>
      )}
        ...
        <>
          {viewMode === "weekly" && (
            <div className="flex justify-end gap-2">
              ...
              <span className="font-medium">{visibleDays[0]}–{visibleDays[visibleDays.length-1]}</span>
              <Button onClick={nextWeek} disabled={currentWeekStart + 6 >= daysInMonth}>Next week</Button>
            </div>
          )}
        ...
        <table className="min-w-full border-collapse">
          <thead>
            <tr>
              ...
              {visibleDays.map(d => <th key={d} className="border p-2 text-center">{d}</th>)}
          ...
          <tbody>
            ...
            {workerNames.length === 0 && (
              <tr>
                <td colSpan={visibleDays.length + 2} className="text-center text-gray-500 p-6">
